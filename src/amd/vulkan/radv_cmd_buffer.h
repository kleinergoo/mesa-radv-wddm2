...
/* MSVC-compatible compilers (e.g. clang-cl / zig cc -target x86_64-windows-msvc,
 * used by the Windows builds) use a 32-bit enum base type. Constants with a
 * value >= 2^32 are silently truncated to zero, and a member placed exactly at
 * bit 31 becomes INT_MIN whose complement (0x7fffffff) zero-extends and wipes
 * every higher bit out of any mask it is AND'ed into. Define every member with
 * bit >= 31 as a 64-bit macro so the bits survive those compilers.
 */
...
/* Same 32-bit-enum workaround as RADV_DYNAMIC_* above (see note there). */
#define RADV_CMD_DIRTY_TESS_DOMAIN_ORIGIN_STATE (1ull << 31)
#define RADV_CMD_DIRTY_LS_HS_CONFIG (1ull << 32)
#define RADV_CMD_DIRTY_VGT_PRIM_STATE (1ull << 33)
#define RADV_CMD_DIRTY_FORCE_VRS_STATE (1ull << 34)
#define RADV_CMD_DIRTY_NGGC_VIEWPORT (1ull << 35)
#define RADV_CMD_DIRTY_NGGC_SETTINGS (1ull << 36)
#define RADV_CMD_DIRTY_PS_EPILOG_SHADER (1ull << 37)
#define RADV_CMD_DIRTY_PS_EPILOG_STATE (1ull << 38)
#define RADV_CMD_DIRTY_GFX12_HIZ_WA_STATE (1ull << 39)
#define RADV_CMD_DIRTY_OVERRIDE_VRS_STATE (1ull << 40)
#define RADV_CMD_DIRTY_ALL ((1ull << 41) - 1)
